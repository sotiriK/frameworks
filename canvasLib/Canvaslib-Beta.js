...
(function () {
    ...
    var mBeforeFrame = null, mOnFrame = null, mAfterFrame = null, mOnLoaded = null, mOnAudioError = null;
    ...
    var mBrokenAudio = function (nError) {
        //This will occur in some browsers when running on a system that has no audio capabilities
        mSounds = {};
        if (mOnAudioError) mOnAudioError(nError);
    };
    //loop
    var mLoop = function () {
        if (mCheckLoaded) {
    ...
    mLib.setVolume = function (nVolume, nName) {
        ...
        } else if (mSounds[nName]) {
            try {
                mSounds[nName].sound.volume = nVolume;
            } catch (e) { mBrokenAudio(e); }
        }
    };
    mLib.tryLoadSound = function (nName, nSource, nCallback) {
        //Does not participate in onLoaded, use nCallback or isSoundLoaded
        //nCallback is a function with one parameter:
        //-nName: the name given to the sound
        nName = mString(nName, null);
        nSource = mString(nSource, null);
        if (!nName || !nSource) return;

        var snd = new Audio();
        mSounds[nName] = { sound: snd, loaded: false, batch: 0 };
        snd.addEventListener("canplaythrough", function () {
            if (!mSounds[nName] || mSounds[nName].loaded) return;
            mSounds[nName].loaded = true;
            if (typeof nCallback === "function") nCallback(nName);
        });
        try {            
            snd.autoplay = false;
            snd.volume = val.volume;
    ...
    mLib.onAudioError = function (nHandler) {
        //nHandler is a function with one parameter:
        //-error: the error reported by the browser, see mBrokenAudio()
        if (!nHandler) { mOnAudioError = null; return; }
        if (typeof nHandler === "function") mOnAudioError = nHandler;
    };
    //
    ...
    var canvasLoop = (function () {
        inherit(canvasAnime, canvasLoop);
        function canvasLoop(nSprites, nDelay, nBackplay) {
            this.constructor.parent.call(this);
            this.mBeginLoop = function () { this.constructor.parent.prototype.begin.call(this, nSprites, nDelay, nBackplay, true); };
        }
    ...
    var canvasAnimeSheet = (function () {
        inherit(canvasAnime, canvasAnimeSheet);
        ...
        canvasAnimeSheet.prototype.begin = function (nSpriteSheet, nColumns, nRows, nDelay, nBackplay, nRepeat, nRepeatCount) {
            ...
            this.constructor.parent.prototype.begin.call(this, [nSpriteSheet], nDelay, nBackplay, nRepeat, nRepeatCount);
        };
    ...
    var canvasLoopSheet = (function () {
        inherit(canvasAnimeSheet, canvasLoopSheet);
        function canvasLoopSheet(nSpriteSheet, nColumns, nRows, nDelay, nBackplay) {
            this.constructor.parent.call(this);
            this.mBeginLoop = function () { this.constructor.parent.prototype.begin.call(this, nSpriteSheet, nColumns, nRows, nDelay, nBackplay, true); };
        }
    ...
    mWin.canvasLib.inherit = inherit;
})();
