...
(function () {
    ...
    var mLandApplyMinRows = false, mLandApplyMinCols = false, mPortApplyMinRows = false, mPortApplyMinCols = false;
    ...
    var mPageStartWidthPx = function () {
        //min cells apply only when reach min pixel size, unless mLandApplyMinCols/mPortApplyMinCols
        var min;
        if (mLandscape)
            min = mMinLandColumns * (mLandApplyMinCols ? mCurrCellSizePx : mMinCellSizePx);
        else min = mMinPortColumns * (mPortApplyMinCols ? mCurrCellSizePx : mMinCellSizePx);
        return mWin.innerWidth > min ? mWin.innerWidth : min;
    };
    var mPageStartHeightPx = function () {
        //min cells apply only when reach min pixel size, unless mLandApplyMinRows/mPortApplyMinRows
        var min;
        if (mLandscape)
            min = mMinLandRows * (mLandApplyMinRows ? mCurrCellSizePx : mMinCellSizePx);
        else min = mMinPortRows * (mPortApplyMinRows ? mCurrCellSizePx : mMinCellSizePx);
    ...
    var mPositionColumnPercent = function (nHolder, nField, ct) {
        ...
        var h = mHolder(mElement(ct[1]), true); if (!h) h = jPage;
        //check for column or width dependent
        var id = mHolderElementId(nHolder), dep = mLandscape ? prop.landWidth : prop.portWidth;
        if (!h || mHasDependent(id, nField, h) || mHasDependent(id, dep, h)) { nHolder[nField] = 0; mWaiting = true; return; }
        //after derived column
        var dc = (h == jPage ? 0 : h[prop.renderColumn]); if (dc == null) { mWaiting = true; return; }
        //derived width
        var dw = (h == jPage ? mColumnsCount : h[prop.renderWidth]); if (dw == null) { mWaiting = true; return; }
    ...
    var mPositionRowPercent = function (nHolder, nField, rt) {
        ...
        var h = mHolder(mElement(rt[1]), true); if (!h) h = jPage;
        //check for row or height dependent
        var id = mHolderElementId(nHolder), dep = mLandscape ? prop.landHeight : prop.portHeight;
        if (!h || mHasDependent(id, nField, h) || mHasDependent(id, dep, h)) { nHolder[nField] = 0; mWaiting = true; return; }
        //after derived row
        var dr = (h == jPage ? 0 : h[prop.renderRow]); if (dr == null) { mWaiting = true; return; }
        //derived height
        var dh = (h == jPage ? mRowsCount : h[prop.renderHeight]); if (dh == null) { mWaiting = true; return; }
    ...
    //main    
    mGrid.min = function (nColumns, nRows, nAlwaysCols, nAlwaysRows) {
        mGrid.minLandscape(nColumns, nRows, nAlwaysCols, nAlwaysRows);
        mGrid.minPortrait(nColumns, nRows, nAlwaysCols, nAlwaysRows);
    };
    ...
    //context    
    mGrid.minLandscape = function (nColumns, nRows, nAlwaysCols, nAlwaysRows) {
        mMinLandColumns = mNumber(nColumns, mMinLandColumns);
        mMinLandRows = mNumber(nRows, mMinLandRows);
        mLandApplyMinCols = !!nAlwaysCols;
        mLandApplyMinRows = !!nAlwaysRows;
    };
    mGrid.minPortrait = function (nColumns, nRows, nAlwaysCols, nAlwaysRows) {
        mMinPortColumns = mNumber(nColumns, mMinPortColumns);
        mMinPortRows = mNumber(nRows, mMinPortRows);
        mPortApplyMinCols = !!nAlwaysCols;
        mPortApplyMinRows = !!nAlwaysRows;
    };
